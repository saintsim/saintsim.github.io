...
function updateBlock(blockName, elementName, boxName, iconName, currentHour, response) {
    const block = getTemperatureBlock(blockName, response.hourly, currentHour);
    {
        ...
        setElementBlock$1(elementName + "TempMaxFeelsLike", `${tempMaxFeelsLikeString}`);
        setElementBlock$1(elementName + "CurrentConditions", `${conditionsString}`);
        setElementBlock$1(elementName + "CurrentRain", `${percString}`);
    }
}
function updateCurrentBlock(response, weatherCode) {
    setElementBlock$1("currentTemperatureFeelsLike", `${Math.round(response.current.apparent_temperature)} °C`);
    setElementBlock$1("currentConditions", getWeatherDescription(weatherCode));
...
function updateChanceOfRainBlock() {
    ...
    setElementBlock$1("carryUmbrealla", `${chanceOfRainStr}`);
